...
We present here a dataset of 79,890 entangled sequences collected through large-scale sequence screening, ESMFold structure prediction and a series of data cleaning steps. Sequences and all other information (e.g. functions, organisms, calculated binding energies) are availbale at `data/Uniref50_entangled_homodimer.csv`. Scripts for the structure prediction and metric calculation are available at `screening/`. The ESMFold-predicted homodimeric structures are available at FigShare (https://doi.org/10.6084/m9.figshare.29604998.v1).

We provide a conda environment for the easy access. You can install [conda](https://docs.conda.io/projects/conda/en/latest/user-guide/install/) first and then install TangleDiff and its dependencies through:
...
You can also train the model with your own dataset by replacing `data/Uniref50_entangled_homodimer.csv` with your own. To train TangleDiff with customized molecular-level properties, we can replace the core_binding_energy_relax_pack column in `data/Uniref50_entangled_homodimer.csv` with your own properties and set the class seperation thresholds through the `data.binding_energy_seps` argument.
